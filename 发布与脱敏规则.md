...
首次发布前，仓库管理员必须在 **Settings -> Pages -> Build and deployment** 中将 Source 设为 **GitHub Actions**。如果站点尚未启用，`actions/configure-pages` 读取 Pages 配置会返回 `Not Found`，此时无需修改 VuePress 内容或放宽公开扫描规则。

`actions/configure-pages@v6` 已使用 Node.js 24。该 Action 的自动启用能力需要具有仓库管理和 Pages 写权限的专用令牌，默认 `GITHUB_TOKEN` 不具备该权限；本仓库采用管理员手动完成一次启用、工作流后续持续发布的方式。

### 配置修改要求

- 修改私有目录或私有笔记清单时，必须同时更新 `.gitignore`、本地私有清单、`.vuepress/config.ts`、`.vuepress/private-content.ts` 和 `scripts/check-public-content.mjs`；VuePress 的页面构建与顶部导航都必须保持同一套公开边界，不能只屏蔽页面而遗漏导航链接。
